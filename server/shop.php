<?php
	...
	$query2 = "SELECT DATE_FORMAT(FROM_UNIXTIME(`DATE`), '%d.%m.%Y') as Dat,
						CARD.CODE, ID_CHEQUE, ROUND(SUM(SUM_SALE), 2) as SUMCHEK
					FROM `SALE` 
					LEFT OUTER JOIN CARD on (SALE.ID_CARD = CARD.ID)
					WHERE `DATE` >= UNIX_TIMESTAMP('".$_POST['dat1']."')
						and `DATE` <= UNIX_TIMESTAMP('".$_POST['dat2']."')".$str."
					GROUP BY ID_CHEQUE
					ORDER BY CARD.CODE";
	...
	foreach ($resp as $item) {
		echo $item['CARD'].",".$item['CHECK'].",".$item['SUMMA'].",".$item['CHECKSUM']."/";
	}
?>